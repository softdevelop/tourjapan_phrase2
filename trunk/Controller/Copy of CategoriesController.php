...
class CategoriesController extends AppController {
	...
	public function filter()
	{
		...
		if (!$this->Session->check('catId') || !empty($_POST['catId'])) 
		{
			$ids = !empty($_POST['catId']) ? $_POST['catId'] : array();	
			
			$this->Session->write('catId', $ids);
			}
			$tours = $this->TourPackagesCategory->find('list', array(
				'fields' => array('TourPackagesCategory.tour_package_id'),	
				'conditions' => array('TourPackagesCategory.category_id' => $this->Session->read('catId')),
    ...
    'hasOne' => array(
		...
		$cat_name = $this->Category->find('list', array(
				'conditions' => array('Category.id' => $_POST['catId'])
			));
		$catname = implode( '・', $cat_name );
		$postdata['catname'] = $catname;
		$postdata['catId'] = $_POST['catId'];
		}else{
		$postdata['catname'] = "無し";
		}
