<article id="toplist">
		...
		<?php if (!empty($tours)) :
			...
			?>
			<?php  if(strstr($_SERVER["REQUEST_URI"], '/featureds')){ ?>
				<p class="cond_s">お気に入りツアー</p>
				<?php }elseif(isset($postdata)){ ?>
		<p class="cond_s">エリア／<?php echo $postdata['areaname'];?>　　ジャンル／<?php echo $postdata['catname']; ?></p> 
	<?php } ?>
