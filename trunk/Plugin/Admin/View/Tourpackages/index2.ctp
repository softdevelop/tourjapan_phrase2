<form action="/admin/tourpackages" method="get">
			...
			<thead>
				...
				<tr role="row" class="filter">
					<td rowspan="1" colspan="1">
						<input type="text" class="form-control form-filter input-sm" name="data[tour_id]">
					</td>
					<td rowspan="1" colspan="1">
						<input type="text" class="form-control form-filter input-sm" name="data[tour_name]">
					</td>
					<td rowspan="1" colspan="1">
						<input type="text" class="form-control form-filter input-sm" name="data[sponsor_id]">
					</td>
					<td rowspan="1" colspan="1">
						<input type="text" class="form-control form-filter input-sm" name="data[sponsor_name]">
					</td>
					<td rowspan="1" colspan="1">
						<select name="status">
						...
						<tbody role="alert" aria-live="polite" aria-relevant="all">
							...
							?>
								<tr class="<?php if ( $i % 2 != 0 ): ?> odd <?php else: ?> even <?php endif;?>">
									...
									<td class=" ">
										<?php echo $tourPackage['Sponsor']['id']; ?>
									</td>
									<td class=" ">
										<?php echo $tourPackage['Sponsor']['sponsor_name']; ?>
									</td>
									<td class=" ">
										<?php echo $tourPackage['Area']['name']; ?>
									</td>
									<td class=" ">
										<?php 
										   
											$today = strtotime(date('Y-m-d'));
											$res = strtotime($tourPackage['TourPackage']['schedule_start']);
											$end =  strtotime($tourPackage['TourPackage']['schedule_end']);
											if($tourPackage['TourPackage']['status'] == '0'){
									...
									<td class=" ">
										<?php 
											echo $this->Html->link(
											    '編集',
											    array('controller' => 'tourpackages', 'action' => 'edit', $tourPackage['TourPackage']['id'])
											);
										?>
									</td>
									<!-- <td class=" ">
